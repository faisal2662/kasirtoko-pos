            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <ul class="list-group mt-3" id="menuSortable">
                            @foreach ($menus as $menu)
                                <li class="list-group-item" data-id="{{ $menu->id }}">

                                    <i class="{{ $menu->icon }} me-2"></i>
                                    {{ $menu->name }} <span
                                        style="margin-left:300px; padding-left: 10px; border-left:1px solid #000;">
                                        {{ $menu->route }} </span> <span class="float-end"> <a href="#"
                                            onclick="formUpdate({{ $menu->id }}, '{{ $menu->name }}' , '{{ $menu->icon }}', '{{ $menu->route }}', {{ $menu->header }}, '{{ $menu->parent_id }}')"><span
                                                class="badge bg-warning">
                                                <i class="bi bi-pencil-square"></i> </span></a> || <span
                                            class="badge bg-danger" onclick="hapus_menu({{ $menu->id }})"
                                            style="cursor:pointer;"><i class="bi bi-trash3"></i></span> </span>

                                    @if ($menu->children->count())
                                        <ul class="list-group mt-2 submenu-sortable">
                                            @foreach ($menu->children as $child)
                                                <li class="list-group-item" data-id="{{ $child->id }}">
                                                    <i class="{{ $child->icon }} me-2"></i>
                                                    {{ $child->name }} <span
                                                        style="margin-left:300px; padding-left:10px; border-left:1px solid #000;">
                                                        {{ $child->route }} </span> <span class="float-end"> <a
                                                            href="#"
                                                            onclick="formUpdate({{ $child->id }}, '{{ $child->name }}' , '{{ $child->icon }}', '{{ $child->route }}', {{ $child->header }}, '{{ $child->parent_id }}')"><span
                                                                class="badge bg-warning"> <i
                                                                    class="bi bi-pencil-square"></i> </span></a> ||
                                                        <span class="badge bg-danger" style="cursor: pointer;"
                                                            onclick="hapus_menu({{ $child->id }})"><i
                                                                class="bi bi-trash3"></i></span> </span>
                                                </li>
                                            @endforeach
                                        </ul>
                                    @endif
                        ...
                        <button class="btn btn-primary mt-3" id="saveOrder">
                            Simpan Urutan
                        </button>
